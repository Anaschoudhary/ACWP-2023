...
function acwp_posted_on(){
    ...
    $posted_on = sprintf(
        esc_html_x('Posted on %s', 'post date', 'acwp2023'),
...
function acwp_posted_by(){

    $byline = sprintf(
        esc_html_x(' by %s', 'post author', 'acwp2023'),
        '<span class="author vcard"><a href="'.esc_url(get_author_posts_url(get_the_author_meta('ID'))).'">'.esc_html(get_the_author()).'</a></span>'
    );

    echo '<span class="by-line text-secondry">'.$byline.'</span>';
}

function acwp_excerpt($trim_char_count = 0) {
